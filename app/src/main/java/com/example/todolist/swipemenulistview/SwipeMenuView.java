...
public class SwipeMenuView extends LinearLayout implements OnClickListener {
	...
	private void addItem(SwipeMenuItem item, int id) {
		...
		if (item.getBackgroundColor() != 0) {
			parent.setBackgroundColor(item.getBackgroundColor());
		}
		if (item.getBackgroundResource() != 0) {
			parent.setBackgroundResource(item.getBackgroundResource());
		}
	...
	private ImageView createIcon(SwipeMenuItem item) {
		...
		if (item.getIconColor() != 0) {
			iv.setColorFilter(item.getIconColor(), PorterDuff.Mode.SRC_IN);
		}
		return iv;
	}
